...
export const LeadForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const waNumber = content.contact.whatsapp.replace(/\s+/g, "");
        if (waNumber) {
  ...
  if (isSuccess) {
    return (
      <section id="teklif-al" className="container mx-auto px-4 md:px-6 py-16 md:py-20">
      ...
      <div className="max-w-4xl mx-auto rounded-[3rem] md:rounded-[4rem] bg-slate-900 p-1 md:p-2 shadow-2xl shadow-primary-ocean/20">
        <div className="bg-white rounded-[2.8rem] md:rounded-[3.5rem] p-6 sm:p-8 md:p-16 space-y-8 md:space-y-12">
          <div className="text-center space-y-4">
            <span className="text-[10px] font-black text-primary-ocean uppercase tracking-[0.3em] bg-primary-ocean/5 px-6 py-2 rounded-full">
              Hızlı Teklif Sistemi
            </span>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 italic tracking-tight">
              Hemen Randevu Alın
            </h2>
            <p className="text-sm md:text-base text-slate-400 font-medium max-w-md mx-auto italic">
              Formu doldurun, uzman ekibimiz sizi 15 dakika içinde arasın.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <User
                    className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-primary-ocean transition-colors"
                    size={20}
                  />
                  <input
                    id="fullName"
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <MessageSquare
                    className="absolute left-6 top-6 text-slate-300 group-focus-within:text-primary-ocean transition-colors"
                    size={20}
                  />
                  <textarea
                    id="notes"
                    ...
                    rows={1}
                    className="w-full pl-14 pr-8 py-5 rounded-2xl bg-slate-50 border border-slate-100 outline-none focus:bg-white focus:border-primary-ocean transition-all font-bold text-slate-700 resize-none disabled:opacity-60"
                  />
                </div>
              </div>
            </div>
